...
<!--              COLUMN HEADER AND CELL DEFINITIONS    -->
              <ng-container *ngFor='let column of columnObjects;let i = index' cdkColumnDef='{{column.columnDef}}'>
                <th scope="col" class='sticky top-0 z-10 border-b border-primary-dark bg-secondary-light bg-opacity-75 py-sm pl-sm pr-sm text-left text-sm font-semibold text-black backdrop-blur backdrop-filter sm:pl-sm lg:pl-lg' cdk-header-cell *cdkHeaderCellDef>{{column.header}}</th>
                <td class="whitespace-nowrap border-b border-primary-light py-sm pl-sm pr-sm text-sm font-medium text-black sm:pl-sm lg:pl-lg" cdk-cell *cdkCellDef='let row'>{{column.cell(row)}}</td>
              </ng-container>
            </table>
          </div>
        </div>
